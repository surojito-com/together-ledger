...
test('SMTP messages contain only the intended single-use application links', async () => {
  ...
  const mailer = new SmtpMailer({ transport, from: 'Together Ledger <no-reply@example.test>', publicOrigin: 'https://together.example.test' });
  ...
  assert.match(messages[0].text, /https:\/\/together\.example\.test\/\?verify=verify_token/);
  assert.match(messages[1].text, /https:\/\/together\.example\.test\/\?invite=invite_token/);
  assert.match(messages[2].text, /https:\/\/together\.example\.test\/\?recovery=recovery_token/);
