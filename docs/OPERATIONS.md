...
3. Set `CADDY_DOMAIN=api.together-ledger.com` and `API_ORIGIN=https://api.together-ledger.com` only after staging DNS and TLS are ready. Keep `PUBLIC_ORIGIN=https://together-ledger.com`, and set `ACCOUNT_ORIGIN=https://together-ledger.com` once that public home is account-capable. `ACCOUNT_ORIGIN` is the safe fallback for trusted non-browser jobs. Browser-issued verification, recovery, and invitation emails preserve the already allowlisted origin that requested them: public-home actions return to the public home, direct API testing returns to the API client, and arbitrary origins are rejected before mail is sent.
